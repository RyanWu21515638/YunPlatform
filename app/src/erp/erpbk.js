...
erp.controller('ErpCtrl', function ($rootScope, $scope, $timeout, $interval, $state, $cookies, $http,erpService) {
        ...
        $scope.userinfo.signature = $cookies.get('signature');
                ...
                $scope.cutrecord = function (y, p, p_number, partition) {
                    ...
                    $http.post($rootScope.ip + "/rlerp/index.php/home/storage/postUpdate_Incise", $scope.form, $scope.postCfg).success(
                        function (res) {
                            if (res.error == '998') {
                                $scope.printbl1[y] = 1;
                                alert('已经开料并打印过条码！');
                ...
                $scope.punchrecord = function (y, x) {
                    ...
                    $scope.form.userid = $cookies.get('userid');
                    $scope.form.project_id = $scope.project_id;
                    $http.post($rootScope.ip + "/rlerp/index.php/home/storage/postUpdate_punch", $scope.form, $scope.postCfg).success(
                        function (res) {
                            if (res.error == '997')
                            {
                ...
                $scope.jointrecord = function (y, x) {
                    ...
                    $scope.form.userid = $cookies.get('userid');
                    $scope.form.project_id = $scope.project_id;
                    $http.post($rootScope.ip + "/rlerp/index.php/home/storage/postUpdate_Weld", $scope.form, $scope.postCfg).success(
                        function (res) {
                            if (res.error == '996') {
                ...
                $scope.feedrecord = function (y, x) {
                    ...
                    $scope.form.userid = $cookies.get('userid');
                    $scope.form.project_id = $scope.project_id;
                    $http.post($rootScope.ip + "/rlerp/index.php/home/storage/postUpdate_Material", $scope.form, $scope.postCfg).success(
                        function (res) {
                            if (res.error == '996') {
                                document.getElementById("labelmsg").style.display = 'none';
                                document.getElementById("labelmsg2").style.display = 'block';
